<template>
  <div class="pe-bg-black">
    ...
    <section
      v-if="hasPaidReservations || isOnHoldReservation"
    ...
    >
      ...
      >
      <p class="pe-text-center pe-text-[24px] pe-mb-[60px]"> {{ $t('subtitle') }}</p>
...
import i18n from '../i18n/confirmations/onHold.lang'
...
interface reservationInfoDetail {
  room: string
  name: string
  roomName: string
...
export default class ResortCreditPage extends Mixins(BookingWidgetMixin) {
  ...
  reservationDefault: reservationInfoDetail = {
    room: '',
    name: '',
    roomName: '',
    confirmationId: '',
  ...
  formatedReservations: reservationInfoDetail[] = []
  ...
  mounted() {
    this.formatedReservations.forEach((reservation: reservationInfoDetail) => {
      ...
      const justBookItEvent: JustBookItDataLayer = {
        event: 'justBookIt',
        hotelName,
        ...
        totalRooms,
        totalServices,
        reservID: hotelName + '-' + reservation.confirmationId,
        rooms
      }

      // console.log('justBookItEvent', justBookItEvent)

      // @ts-ignore
      this.$gtm.push(justBookItEvent)
    })
  }
}
</script>
