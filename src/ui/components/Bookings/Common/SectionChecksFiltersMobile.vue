...
@Component({
  i18n,
  name: 'BookingSectionFiltersMobile'
})
export default class BookingSectionFiltersMobile extends Vue {
  ...
  created() {
    this.filtersSelected = this.filtersItems.map(({name}: FilterSection, index) => {
      return {
        name,
