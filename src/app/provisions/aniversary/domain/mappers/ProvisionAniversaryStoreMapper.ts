...
const filterStoreProvitions = (provisions: ProvisionStore[]): ProvisionStore[] => {
  ...
  let weeks = provisions.filter( item =>  item.category === 'ANVWKS')
  ...
  let nights = provisions.filter( item => item.category !== 'ANVWKS' )
...
export const provisionAniversaryStoreMapper = (provition: any[]): ProvitionOptionItem[] => {
  ...
  const buildName = (item: ProvisionStore, index: number): string => {
    ...
    result = item.hasGolf ? `${result} Unlimited Golf With Additional Payment` : result
    return `Anniversary ${result}`
  }
