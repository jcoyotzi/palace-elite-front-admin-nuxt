import {ReservationReferral} from "app/booking/domain/entities/reservationReferral";

export interface Provition {
  Incentive:             Incentive[];
  Anniversary:           Anniversary[];
  ReservationsReferrals: ReservationsReferrals;
  TemporalBenefits:      any[];
  Promotions:            Promotion[];
}

export interface Anniversary {
  Agency_cd_ext:            string;
  Agency_cd_nac:            string;
  Contract_ext:             string;
  Contract_nac:             string;
  Cdeaccshtl:               string;
  Cdeaccsglf:               string;
  Accesshotel:              string;
  Accessgolf:               string;
  NumberAirfare:            string;
  PriceAirfare:             number;
  AmountTaxFee:             number;
  Category:                 string;
  IsLBC:                    boolean;
  IsResidence:              boolean;
  ProvPack:                 number;
  ExceededAirfare:          number;
  TaxFeeProvis:             number;
  CostProvis:               number;
  AccessThanksGiving:       boolean;
  IsRestrictUse:            boolean;
  RECNUM:                   number;
  COMPANY:                  number;
  APPLICATION:              string;
  TYPE:                     string;
  FOLIO:                    number;
  FIRST:                    string;
  LAST:                     string;
  SALE_DTE:                 string;
  USG_DTE:                  string;
  ADDRESS:                  string;
  CITY:                     string;
  STATE:                    string;
  ZIP:                      string;
  CNTRY:                    string;
  TEL:                      string;
  USG_PLACE:                string;
  MPAYS:                    number;
  USG_TEMP:                 string;
  VENC_YRS:                 number;
  SALIDA:                   string;
  APP_AMIGO:                string;
  KIND:                     string;
  HOTEL:                    string;
  HOTFOLIO:                 number;
  RM_TYPE:                  string;
  ARV_DATE:                 string;
  STATUS:                   string;
  INVOICE:                  string;
  VALIDYRS:                 number;
  VALIDDTE:                 string;
  IDCHECKIN:                number;
  CONTRACT:                 string;
  IGR:                      string;
  IGRN:                     number;
  IGRTYPEWK:                string;
  IGRFOLIOWK:               number;
  TAR_PAGADA:               number;
  INVOICEDTE:               string;
  FOLIOAIRT:                string;
  CO_AMIGO:                 number;
  REWARDDATE:               string;
  WEEK52:                   string;
  STOCK_WEEK:               string;
  STOCK_WK_TYPE:            string;
  GR_INCLUDE:               number;
  ULG_INCLUDE:              string;
  ULG_AMOUNT:               number;
  ULG_MAX_PERS:             number;
  PROJECTID:                number;
  TYPENAME:                 string;
  WEEK52_START_YEAR:        number;
  WEEK52_VALIDITY_YEARS:    number;
  ChristmasBonus:           boolean;
  ADDITIONALBENEFITS:       null | string;
  Stock:                    any;
  StockUnit:                any;
  IncentiveWeek:            any;
  Cash_sale:                any;
  NumberExtraValidityYears: number;
  IsIncongruos:             boolean;
  StockExtraValidityYears:  boolean;
  ULG_ADDITIONAL_PAYMENT:   boolean;
  MinimumOccupancy:         number;
  MaximumOccupancy:         number;
  ReplacementType:          number;
}

export interface Incentive {
  Agency_cd_ext:            string;
  Agency_cd_nac:            string;
  Contract_ext:             string;
  Contract_nac:             string;
  Cdeaccshtl:               string;
  Cdeaccsglf:               string;
  Accesshotel:              string;
  Accessgolf:               string;
  NumberAirfare:            string;
  PriceAirfare:             number;
  AmountTaxFee:             number;
  Category:                 string;
  IsLBC:                    boolean;
  IsResidence:              boolean;
  ProvPack:                 number;
  ExceededAirfare:          number;
  TaxFeeProvis:             number;
  CostProvis:               number;
  AccessThanksGiving:       boolean;
  IsRestrictUse:            boolean;
  RECNUM:                   number;
  COMPANY:                  number;
  APPLICATION:              string;
  TYPE:                     string;
  FOLIO:                    number;
  FIRST:                    string;
  LAST:                     string;
  SALE_DTE:                 string;
  USG_DTE:                  string;
  ADDRESS:                  string;
  CITY:                     string;
  STATE:                    string;
  ZIP:                      string;
  CNTRY:                    string;
  TEL:                      string;
  USG_PLACE:                string;
  MPAYS:                    number;
  USG_TEMP:                 string;
  VENC_YRS:                 number;
  SALIDA:                   string;
  APP_AMIGO:                string;
  KIND:                     string;
  HOTEL:                    string;
  HOTFOLIO:                 number;
  RM_TYPE:                  string;
  ARV_DATE:                 string;
  STATUS:                   string;
  INVOICE:                  string;
  VALIDYRS:                 number;
  VALIDDTE:                 string;
  IDCHECKIN:                number;
  CONTRACT:                 string;
  IGR:                      string;
  IGRN:                     number;
  IGRTYPEWK:                string;
  IGRFOLIOWK:               number;
  TAR_PAGADA:               number;
  INVOICEDTE:               string;
  FOLIOAIRT:                string;
  CO_AMIGO:                 number;
  REWARDDATE:               string;
  WEEK52:                   string;
  STOCK_WEEK:               string;
  STOCK_WK_TYPE:            string;
  GR_INCLUDE:               number;
  ULG_INCLUDE:              string;
  ULG_AMOUNT:               number;
  ULG_MAX_PERS:             number;
  PROJECTID:                number;
  TYPENAME:                 string;
  WEEK52_START_YEAR:        number;
  WEEK52_VALIDITY_YEARS:    number;
  ChristmasBonus:           boolean;
  ADDITIONALBENEFITS:       null | string;
  Stock:                    any;
  StockUnit:                any;
  IncentiveWeek:            any;
  Cash_sale:                any;
  NumberExtraValidityYears: number;
  IsIncongruos:             boolean;
  StockExtraValidityYears:  boolean;
  ULG_ADDITIONAL_PAYMENT:   boolean;
  MinimumOccupancy:         number;
  MaximumOccupancy:         number;
  ReplacementType:          number;
}

export interface Promotion {
  Recnum:                         number;
  Code:                           string;
  Description:                    string;
  Message:                        string;
  MinNights:                      number;
  MaxNights:                      number;
  MaxUses:                        number;
  TypePromotion:                  string;
  GroupPromotion:                 string;
  OutTravelWindow:                boolean;
  MinNightsIntoPromotion:         number;
  IsExternal:                     boolean;
  PriorityGroup:                  number;
  Status:                         string;
  OnlyNew:                        number;
  UsesAvailables:                 number;
  PromotionDetail:                any;
  PromotionBookingWindow:         any[];
  PromotionTravelWindow:          any[];
  PromotionHotels:                any[];
  PromotionRooms:                 any;
  PromotionIsNotRefundable:       any;
  PromotionMarkets:               any[];
  PromotionFriendly:              any[];
  PromotionBlackOuts:             any;
  PromotionCondYUses:             any;
  PromotionCategoriesReservation: any;
  PromotionMemberType:            any;
  PromotionCountries:             any;
  PromotionConfiguration:         any;
  PromotionProductType:           any;
  PromotionByProvitionInCategory: any;
  PackageByPromotion:             any[];
  PromotionFriendBlocked:         any;
  PromotionInfoSettings:          any;
  PromotionLevelMember:           any;
  PromotionTransferibleOptions:   any;
  PromotionAddittionalBenefit:    any;
}

export interface ReservationsReferrals {
  RmfolioPromotion: ReservationReferral[];
  Messages:         any;
}
